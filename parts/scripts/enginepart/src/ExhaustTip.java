...
public class ExhaustTip extends EnginePart
{
	...
	public void buildTuningMenu( Menu m )
	{
		...
		s = m.addItem( "X", 1, posX, -1.0, 1.0, 1801, null );
		s.changeVLabelText( Float.toString(posX, "   %1.5f") );
		
		s = m.addItem( "Y", 2, posY, -1.0, 1.0, 1801, null );
		s.changeVLabelText( Float.toString(posY, "   %1.5f") );
		
		s = m.addItem( "Z", 3, posZ, -1.0, 1.0, 1801, null );
		s.changeVLabelText( Float.toString(posZ, "   %1.5f") );
	...
	public void handleMessage( Event m )
	{
		if( m.cmd == 1 )
		{
			...
			((Slider)m.gadget).changeVLabelText( Float.toString(posX, "   %1.5f") );
		}
		
		if( m.cmd == 2 )
		{
			posY = ((Slider)m.gadget).value;
			((Slider)m.gadget).changeVLabelText( Float.toString(posY, "   %1.5f") );
		}
		if( m.cmd == 3 )
		{
			posZ = ((Slider)m.gadget).value;
			((Slider)m.gadget).changeVLabelText( Float.toString(posZ, "   %1.5f") );
		}
		if( m.cmd == 4 )
		{
		...
		if( m.cmd == 0 )
		{
			...
			m.gadget.osd.findGadget( this, 1 ).changeVLabelText( Float.toString(posX, "   %1.5f") );
			
			m.gadget.osd.findGadget( this, 2 ).setValue( 0.0 );
			m.gadget.osd.findGadget( this, 2 ).changeVLabelText( Float.toString(posY, "   %1.5f") );
			
			m.gadget.osd.findGadget( this, 3 ).setValue( 0.0 );
			m.gadget.osd.findGadget( this, 3 ).changeVLabelText( Float.toString(posZ, "   %1.5f") );
