...
public class DecorativeBodyPart extends BodyPart
{
	...
	{
		super.repair();
		Thread SetPosAgain = new SetAgainDecorPart( posX, posY, posZ, rotY, rotP, rotR, parent, parentSlotID );
		SetPosAgain.start();
		int meshID = getMesh();
		temp_meshRes = new ResourceRef(meshID);
		temp_meshRes.duplicate(temp_meshRes);
...
public class SetAgainDecorPart extends Thread
{
	float posX, posY, posZ, rotY, rotP, rotR;
	Part parent;
	int parentSlotID;

	public SetAgainDecorPart( float posX2, float posY2, float posZ2, float rotY2, float rotP2, float rotR2, Part parent2, int pID )
	{
		posX = posX2;
		posY = posY2;
		posZ = posZ2;
		...
		parent = parent2;
		parentSlotID = pID;
		if(parent instanceof DecorativeBodyPart)
		{
			DecorativeBodyPart parentWing = parent;
		}
	}
