...
{
	float	damping				= 500.0;//set by individual shock // N/(m/s) 
	float	max_damping			= 0.0;	//set by individual shock N/(m/s)
	float	min_damping			= 0.0;	//set by individual shock N/(m/s)
	float	max_length			= 0.45;//	// m 
	float	min_length			= 0.15;//	// m
	float	rebound_factor_interval		= 0.0; //0.0 old
	
	float	rebound_factor_mul 	= 0.6;//new multiplier
	float	damping_mul			= 0.6;
	...
	public void calcStuffs()
	{
		...
		if (whats_inside == WI_GAS)
			rebound_factor = 1.000*rebound_factor_mul; // added multiplier
		else
		if (whats_inside == WI_OIL)
			rebound_factor = 0.750*rebound_factor_mul; // added multiplier
		else
		if (whats_inside == WI_GAS_OIL)
			rebound_factor = 1.333*rebound_factor_mul; // added multiplier

		min_rebound_factor = rebound_factor/rebound_factor_mul*(1.0-rebound_factor_interval);//divided rebound_factor by rebound_factor_mul
		max_rebound_factor = rebound_factor/rebound_factor_mul*(1.0+rebound_factor_interval);//divided rebound_factor by rebound_factor_mul
	...
	public void updatevariables()
	{
		...
		if (whl)
		{
			whl.setDamping(damping*damping_mul, damping*rebound_factor*damping_mul);
